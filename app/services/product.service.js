  ...
  async getNewArrivals(limit = 4) {
    try {
      ...
      const productDetails = products.map(product => {
        const variant = variantMap[product.product_id];
        return {
          name: product.product_name,
          price: variant ? variant.price : null,
          oldPrice: variant ? variant.price * (1 + variant.discount/100) : null,
          image: product.photos[0] || 'https://placehold.co/300x300'
        };
      });
